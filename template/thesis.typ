...
#abstract(
  ...
  针对传统物流管理系统在业务协同、资源调度与路径规划方面的不足，本文设计并实现了一套基于微服务架构的智能物流管理系统。系统围绕订单履约、仓储库存、运输配送与运力调度等核心业务，拆分为认证、用户、订单、库存、运输员、物流规划与网关等独立服务，基于Spring Cloud Alibaba实现服务治理，RabbitMQ负责异步解耦，Redis提供缓存加速。

  系统重点实现了四项内容：基于BCrypt与JWT的统一认证与网关鉴权；围绕订单、库存与配送的异步协同流程；集成GraphHopper路网引擎，实现A\*路径规划、K-Means末端调度预览、MCMF全国运力规划与大语言模型辅助决策；基于Hub-and-Spoke网络的完整履约流程。

  创新点体现在三方面：微服务架构支撑多角色物流协同闭环；真实路网规划、聚类调度与运力优化算法的统一集成；确定性算法求解与生成式模型裁决的受限组合决策模式。
][
  To address the limitations of traditional logistics management systems in business coordination, resource scheduling, and route planning, this thesis designs and implements an intelligent logistics management system based on microservice architecture. The system focuses on order fulfillment, warehousing, transportation, and capacity scheduling, decomposed into independent services for authentication, users, orders, inventory, drivers, logistics planning, and gateway, built on Spring Cloud Alibaba with RabbitMQ for asynchronous decoupling and Redis for caching.

  Key implementations include: unified authentication and gateway authorization based on BCrypt and JWT; asynchronous collaboration for orders, inventory, and delivery; integration of GraphHopper with A-star route planning, K-Means dispatch preview, minimum-cost maximum-flow capacity planning, and LLM-assisted decision-making; and a complete Hub-and-Spoke fulfillment workflow.

  The contributions are threefold: microservice architecture supporting multi-role logistics collaboration; integration of real-road-network planning, clustering dispatch, and capacity optimization into a unified platform; and a constrained combined decision-making mode combining deterministic optimization with generative LLM reasoning.
]
...
在国内，物流信息系统经历了从单机管理软件到平台化云服务的发展过程。在物流信息化平台方面，贾志刚等@ref19 基于Spring Cloud微服务架构对物流运输管理系统进行了重构设计，实现了服务拆分与注册发现；张恩路等@ref20 设计了基于微服务的物流信息平台，但未涉及多角色协同与智能调度。总体来看，国内物流信息化平台多停留在业务流程电子化层面，在服务治理、跨角色协同与智能决策方面的融合仍不充分。

在路径规划与调度算法方面，陈云翔等@ref41 针对物流配送场景提出了改进的A\*路径规划算法，在搜索效率与路径合理性方面取得了一定提升；王勇等@ref39 将K-Means聚类方法应用于物流多配送中心选址优化，为末端配送的站点布局提供了量化依据。然而，上述工作多聚焦于单一算法优化，缺乏将路径规划与末端调度在同一系统中联动的工程实践。

在运力优化方面，赵鹏等@ref43 针对城际物流场景构建了基于Hub-and-Spoke网络的运力优化模型，但未涉及与最小费用最大流算法的结合，亦未在微服务架构下实现部署与跨服务协同。根据中国物流与采购联合会数据@ref40，2023年社会物流总费用占GDP比重仍高于发达国家水平，反映出系统性优化的迫切性。综合来看，国内现有工作多集中于单一功能模块优化，缺乏将路径规划、调度执行与运力规划统一的综合性研究，大语言模型在物流决策领域的工程化实践亦处于起步阶段。

=== 国外研究现状

在国外，物流与供应链领域的微服务架构研究已较为深入。Gonzalez等@ref33 系统分析了从传统ERP向微服务架构迁移的策略与挑战，指出服务拆分粒度与数据一致性是迁移的关键难题；Liu等@ref31 设计了基于微服务的智能物流管控平台，验证了微服务在物流场景中的可行性。这些工作偏重架构层面，对智能算法与业务流程的深度融合关注不足。

在路径规划算法方面，Zhang等@ref34 提出了改进的A\*算法用于真实路网环境下的车辆路径规划，验证了启发式搜索在物流场景中的实用性；Liu等@ref35 对VRP模型与算法进行了系统综述，将求解方法分为精确算法与元启发式算法两大类。然而，现有路径规划研究多集中于算法本身优化，较少在真实路网数据与微服务架构相结合的工程环境中进行验证与裁决。

在运力优化与智能决策方面，Zhao等@ref36 构建了考虑流量延迟成本的Hub-and-Spoke网络优化模型，为干线运输网络设计提供了数学规划工具；Fragkos等@ref37 对最小费用最大流精确算法进行了综述，比较了多种网络流求解方法的适用场景与计算复杂度。近年来，Kumar等@ref42 系统梳理了机器学习在物流与供应链管理中的应用，指出LLM等生成式模型在需求预测与决策辅助方面具有潜力；Simchi-Levi等@ref38 探讨了LLM在供应链决策中的具体应用场景。然而，现有工作尚未形成"确定性算法+生成式模型"的混合决策模式，运力规划与调度的工程实践仍以传统运筹方法为主。
...
如图所示，发货用户需要地址管理、发起寄件、平台下单、查看物流单等功能。
...
基于上述架构，系统核心业务围绕"下单→调度→干线→中转→末端→签收"的Hub-and-Spoke履约链路展开。图@order-flow 展示了从发货用户下单到收件人签收的完整业务流程，涉及订单服务、物流服务、运输员服务三个核心服务的跨服务协作。
...
图中横向按“下单受理→仓配建模→调度决策→干线履约→到站激活→末端签收”六个阶段组织，纵向按参与角色、订单服务、物流服务和运输员服务划分泳道。订单创建后，订单服务通过Feign同步调用物流服务完成起点仓与终点Hub分配，再通过RabbitMQ异步写入调度池；管理员基于K-Means预览结果确认批次后，系统发起干线运输，并在到站后以消息方式激活末端任务，最终由运输员完成配送并闭环签收。
...
本模块实现发货用户的个人信息与地址管理、商户的承运物管理、仓库管理与库存维护等功能。模块设计遵循跨服务协同原则：下单链路采用同步校验（验证库存与地址有效性）+ 异步扣减（通过RabbitMQ解耦库存扣减与配送创建）的混合模式，确保数据一致性的同时避免服务间同步阻塞导致的性能瓶颈。在库存扣减方面，系统采用乐观锁机制防止超卖——更新库存时附加版本校验条件，若并发冲突则由业务层重试，在低并发场景下优于分布式锁的性能开销。个人与地址间为一对多关系，商户、承运物与仓库间涉及多对多关系，通过关系表维护。以下分别给出核心实体与接口设计。
...
本模块涵盖运输员管理、车辆管理、仓储管理与物流单管理四个子功能，是连接订单履约与配送执行的核心业务环节。配送流程采用状态机设计：配送单状态从待接单(0)经已接单(1)、运输中(2)到达已送达(3)完成履约，4=已取消为异常终止态，每次状态流转均由运输员主动触发或系统自动判定并记录时间戳，确保配送全流程可追溯。运输员与车辆间为多对多关系，通过运输员-车辆关系表维护，支持同一运输员在多次配送中使用不同车辆的灵活场景。配送任务作为连接运输员与包裹的核心实体，其创建由物流服务在调度执行时通过RabbitMQ异步通知触发，实现调度决策与配送执行的解耦。
...
调度与运力规划模块是系统智能化能力的核心承载，负责路径规划、中转站管理、调度执行与全国运力规划@ref36 @ref37。模块采用Hub-and-Spoke网络模型作为顶层设计范式：将全国物流节点按层级划分为全国枢纽、省级中心与城市配送中心三级，通过中转站间的运输连接构建干线网络，末端配送则由K-Means聚类生成的配送批次覆盖。这一模型选型基于以下考量：Hub-and-Spoke通过集中转运实现了规模效应，干线运输与末端配送的职责分离使两种场景可采用差异化的优化算法（MCMF用于干线、K-Means用于末端），同时三级中转站结构天然映射到MCMF建模范式中节点与边的抽象。调度执行采用"调度池→聚类预览→批次执行"三步流程：待配送订单首先入调度池，管理员可触发K-Means聚类预览查看分组效果，确认后执行调度创建配送批次并分配运输员。该模块涉及物流路线、中转站、运输连接、配送批次与调度池等多个数据实体，核心数据表定义如下。
...
+ *聚类执行*：采用K-Means算法，K值由管理员根据当前订单量与可用运输员数指定，未指定时默认取$ceil(sqrt(n))$；距离度量采用Haversine球面距离；初始化采用均匀抽样策略（按等间距索引选取初始中心），减少随机性影响并保证结果可复现；收敛条件为簇分配不再变化或达到100次迭代上限。
...
其余辅助表（customer、shop、product、warehouse、product\_warehouse、address、driver、vehicle、flow\_plan等）的详细字段定义见前文各模块设计小节。
...
该设计将"身份建立"与"访问控制"解耦至不同服务：注册与登录由认证服务独立处理，网关仅负责令牌校验与路径授权，下游服务通过请求头获取用户身份而无需重复解析JWT。系统根据角色类型实施差异化审核策略：普通发货用户注册后可直接使用；商户与运输员需经管理员审核。路径权限规则与角色一一对应：/api/customers/仅允许发货用户、/api/shops/与/api/stocks/仅允许商户、/api/drivers/仅允许运输员、/api/logistics/dispatch/仅允许管理员，形成了"白名单 + 令牌校验 + 路径授权"的组合机制。
...
本论文正文中已按模块完整呈现了核心数据表定义（第3章各模块设计小节）、各模块API接口（各模块API表）及算法参数配置（第4章各算法实现小节）。以下提供辅助数据表的概览索引，供读者快速查阅。

#tablex(
  [customer], [发货用户信息表], [3.2节],
  [address], [地址表], [3.2节],
  [shop\_info], [商户信息表], [3.2节],
  [product], [承运物表], [3.2节],
  [warehouse], [仓库表], [3.2节],
  [product\_warehouse], [库存关系表], [3.2节],
  [driver\_info], [运输员信息表], [3.3节],
  [vehicle], [车辆表], [3.3节],
  [order\_info], [订单表], [3.3节],
  [delivery], [配送单表], [3.3节],
  [hub], [枢纽属性表], [3.4节],
  [hub\_link], [运输连接表], [3.4节],
  [logistics\_route], [物流路线表], [3.4节],
  [logistics\_batch], [配送批次表], [3.4节],
  [logistics\_batch\_item], [批次项表], [3.4节],
  [dispatch\_pool], [调度池表], [3.4节],
  [flow\_plan], [运力规划表], [3.4节],
  [flow\_plan\_item], [规划明细表], [3.4节],
  header: ([表名], [说明], [正文位置]),
