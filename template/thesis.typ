...
物流行业作为支撑国民经济的基础性产业，在电子商务迅猛发展下其重要性日益凸显。然而，传统物流管理模式在业务复杂性、时效性与成本控制方面逐渐暴露出高成本、低效率、信息孤岛等瓶颈。中国的物流成本占GDP比重曾高达14.6%（2022年），远高于发达国家平均水平（约8%）。在物联网、大数据、人工智能等新一代技术驱动下，物流行业正经历数字化和智能化转型@ref1 @ref2 @ref3 @ref4。例如，物联网技术使货物、车辆、仓库等物流要素的状态感知与数据采集成为可能@ref2；大数据技术为路径优化、需求预测提供决策支持@ref3；人工智能在智能调度、风险预警等方面的应用提升了物流系统的自动化水平@ref4。
...
#tablex(
  ...
  [K-Means 调度], [k 值], [管理员指定，未指定时默认$ceil(sqrt(n))$], [末端配送聚类簇数],
  [K-Means 调度], [初始化策略], [均匀抽样], [按等间距索引选取初始中心，减少随机性、保证可复现],
  [K-Means 调度], [收敛条件], [分配不变或迭代 ≥ 100 次], [保证聚类结果收敛],
  [K-Means 调度(末端)], [k 值], [$ceil(N/4)$], [每名末端司机最多4个停靠点],
  [K-Means 调度(末端)], [最大迭代], [50 次], [末端分组场景下减少计算开销],
