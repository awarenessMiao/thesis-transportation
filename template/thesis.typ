...
*MCMF全国运力规划选型方面*，全国干线运力规划将Hub-and-Spoke网络抽象为带容量与费用约束的有向流网络。MCMF作为经典网络流优化算法，能在全局视角下求解最小费用最大流，天然契合Hub-and-Spoke网络的干线运力分配问题@ref37。结合系统中每笔跨城订单都具有明确起始Hub与目标Hub的业务特征，本文采用多商品建模方式，将每个OD对作为独立运输需求，在共享残差容量图上逐OD对寻找低费用路径；边容量取自hub\_link表的日处理能力，边费用取自基准运输单价。该方式避免了将供需简单汇总到Hub级别时可能出现的流向抵消问题，也能为后续干线批次生成保留完整OD路径。此外，系统设计了LLM辅助费率校准机制，用于修正异常干线费率，使规划结果更贴近实际运营经验。
