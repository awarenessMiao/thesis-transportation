...
#imagex(
  scale(x: 90%, y: 90%, dg-layers(
    gap: 1.2em,
    dg-rect(
      header: [#text(weight: "bold")[部署接入层]#linebreak()#text(size: 0.8em)[(Deployment & Access Layer)]],
      header-fill: rgb("#e9d5ff"),
      fill: rgb("#faf5ff"),
      stroke: 1pt + rgb("#d8b4fe"),
      width: 100%,
      dg-flex(
        ...
        justify: "space-around",
        gap: 1em,
        dg-rect([Nginx#linebreak()#text(size: 0.8em)[(静态产物托管)]], fill: white, width: 45%),
        dg-rect([API 反向代理#linebreak()#text(size: 0.8em)[(请求转发至网关)]], fill: white, width: 45%)
      )
    ),
    dg-rect(
      header: [#text(weight: "bold")[视图组件层]#linebreak()#text(size: 0.8em)[(View Component Layer)]],
      header-fill: rgb("#bfdbfe"),
      fill: rgb("#eff6ff"),
      stroke: 1pt + rgb("#93c5fd"),
      width: 100%,
      dg-flex(
        ...
        justify: "center",
        gap: 1em,
        dg-rect([Vue3#linebreak()#text(size: 0.8em)[(核心组件框架)]], fill: white),
        dg-rect([Element Plus#linebreak()#text(size: 0.8em)[(UI 组件库)]], fill: white),
        dg-rect([Leaflet#linebreak()#text(size: 0.8em)[(地图可视化组件)]], fill: white)
      )
    ),
    dg-rect(
      header: [#text(weight: "bold")[路由控制层]#linebreak()#text(size: 0.8em)[(Router Control Layer)]],
      header-fill: rgb("#bbf7d0"),
      fill: rgb("#f0fdf4"),
      stroke: 1pt + rgb("#86efac"),
      width: 100%,
      dg-flex(
        ...
        justify: "space-around",
        gap: 1em,
        dg-rect([Vue Router#linebreak()#text(size: 0.8em)[(单页路由控制)]], fill: white),
        dg-rect([全局路由守卫#linebreak()#text(size: 0.8em)[(登录拦截与角色权限控制)]], fill: white)
      )
    ),
    dg-rect(
      header: [#text(weight: "bold")[状态管理层]#linebreak()#text(size: 0.8em)[(State Management Layer)]],
      header-fill: rgb("#fef08a"),
      fill: rgb("#fefce8"),
      stroke: 1pt + rgb("#fde047"),
      width: 100%,
      dg-flex(
        ...
        justify: "center",
        gap: 1em,
        dg-rect([Pinia#linebreak()#text(size: 0.8em)[(应用核心状态与持久化)]], fill: white, width: 95%)
      )
    ),
    dg-rect(
      header: [#text(weight: "bold")[通信服务层]#linebreak()#text(size: 0.8em)[(Communication Service Layer)]],
      header-fill: rgb("#fecaca"),
      fill: rgb("#fef2f2"),
      stroke: 1pt + rgb("#fca5a5"),
      width: 100%,
      dg-flex(
        ...
        justify: "space-around",
        gap: 1em,
        dg-rect([Axios#linebreak()#text(size: 0.8em)[(API 统一封装)]], fill: white),
        dg-rect([请求/响应拦截器#linebreak()#text(size: 0.8em)[(自动注入 Token 与错误码处理)]], fill: white)
      )
    )
  )),
  caption: [前端总体架构图],
...
系统核心实体按业务域可划分为用户与角色域、商品与库存域、订单与配送域、调度域以及网络与规划域五大部分，如图@er-diagram 所示。在用户与角色域，User表通过permission字段区分四类角色，Customer、Shop与Driver分别通过外键与User形成一对一关联，实现角色公共属性与专属信息的分离；Customer与Address为一对多关系。在商品与库存域，Shop与Product、Warehouse分别为一对多关系，Product与Warehouse通过ProductWarehouse关系表实现多对多关联，以支持同一承运物在多仓库中的分布式库存管理。在订单与配送域，OrderInfo作为核心聚合实体，关联Customer、Shop、Address与Warehouse；Delivery与LogisticsRoute分别关联OrderInfo与Driver，分别承载配送任务与路线规划数据。在调度域，LogisticsBatch包含多个BatchItem，每个BatchItem对应一个末端配送订单；DispatchPool维护待调度订单，为K-Means聚类与批次执行提供输入。在网络与规划域，Hub通过HubLink构成M:N自关联的干线网络拓扑，支撑MCMF运力规划；FlowPlan与FlowPlanItem存储运力规划的全局结果与线路明细。

#imagex(
  image("figures/er-diagram.png", width: 95%),
  caption: [系统核心实体E-R图],
...
// TODO: 需补充仓库与库存管理页面截图（figures/warehouse-inventory.png 当前与 shop-products.png 重复）

=== 运输员端运行结果

运输员端是物流计划落地执行的直接承载端，用于接收配送任务、执行导航与上报送达状态。图@driver-tasks 展示了运输员待处理任务列表，按任务来源（末端配送与直送）分区展示可领取的配送任务，运输员可一键接单并开始执行。接单后，系统基于 GraphHopper 真实路网规划配送路线，在地图页面上渲染途经停靠点与实时位置，运输员按停靠顺序逐单配送并上报送达状态。
...
// TODO: 需补充导航与路线地图页面截图（figures/driver-map.png 当前与 driver-tasks.png 重复且无地图内容）

=== 管理员端运行结果

管理员端集中体现系统的全局管理、智能调度与网络规划能力。在末端调度预览页面，管理员触发调度后，系统对调度池中的订单执行 K-Means 聚类与 TSP 排序，地图上以不同颜色渲染聚类结果，同时展示 LLM 对各聚类的分析建议，确认后执行调度创建批次。在全国网络拓扑页面，系统呈现 Hub 列表、各层级中转站负载统计以及 MCMF 运力分配信息，管理员可据此进行全局运力调控。

// TODO: 需补充末端调度预览页面截图（figures/last-mile-dispatch.png 当前为运输员端页面，与图注不符）
// TODO: 需补充全国网络拓扑与运力规划结果页面截图（figures/national-network.png 当前为运输员端页面，与图注不符）
