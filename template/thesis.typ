...
本模块涵盖运输员管理、车辆管理、仓储管理与物流单管理四个子功能，是连接订单履约与配送执行的核心业务环节。配送流程采用状态机设计：配送单状态从"待承接"(0)经"已接单"(1)、"取件中"(2)、"配送中"(3)最终到达"已送达"(4)，每次状态流转均由运输员主动触发并记录时间戳，确保配送全流程可追溯。运输员与车辆间为多对多关系，通过运输员-车辆关系表维护，支持同一运输员在多次配送中使用不同车辆的灵活场景。配送任务作为连接运输员与包裹的核心实体，其创建由物流服务在调度执行时通过RabbitMQ异步通知触发，实现调度决策与配送执行的解耦。
...
本测试旨在评估 Redis 缓存对高频读接口的性能提升效果，选取仓库列表查询接口（GET /api/shop/{id}/warehouses）作为测试对象。测试前通过 redis-cli DEL 命令清除对应缓存键，确保每次冷请求均为真正的缓存未命中；缓存命中测试则在前序请求已填充缓存后连续发送，保证所有请求均为命中状态。两种状态各发送 50 次请求，记录每次响应时间。
