...
路径规划采用A\*算法作为基础搜索引擎，其代价函数$f(n) = g(n) + h(n)$中启发函数选用Haversine球面距离，满足可采纳性条件。选型依据为：A\*在真实路网数据上可保证最优性，且搜索效率优于Dijkstra等无启发算法；同时引入加权启发策略（Weighted A\*）生成多条差异化候选路线，为后续LLM裁决提供输入多样性。
...
末端配送调度的核心问题是将调度池中的待配送订单按地理位置聚类为若干配送批次。选型依据为：K-Means算法简洁、收敛可预期、适合中等规模地理点集聚类；距离度量采用Haversine球面距离以匹配地理空间特性；聚类完成后对每个簇内部执行近似TSP贪心排序以优化停靠顺序。此外，系统设计了LLM辅助分析环节，为管理员提供簇间合并建议与异常簇识别等辅助决策信息。

==== MCMF全国运力规划选型

全国干线运力规划将Hub-and-Spoke网络抽象为带容量与费用约束的有向流网络。MCMF作为经典网络流优化算法，能在全局视角下求解最小费用最大流，天然契合Hub-and-Spoke网络的干线运力分配问题@ref37。系统采用超级源/汇点建模，将每个Hub拆分为入港与出港两个节点，边容量取自hub\_link表的日处理能力，边费用取自基准运输单价。此外，系统设计了LLM辅助费率校准机制，用于修正异常干线费率，使规划结果更贴近实际运营经验。
...
本论文正文中已按微服务完整呈现了核心数据表定义（第3章各服务设计小节）、各服务API接口（各服务API表）及算法参数配置（第4章各算法实现小节）。以下提供辅助数据表的概览索引，供读者快速查阅。

#tablex(
  [customer], [发货用户信息表], [3.2.3节],
  [address], [地址表], [3.2.3节],
  [shop\_info], [商户信息表], [3.2.4节],
  [product], [承运物表], [3.2.4节],
  [warehouse], [仓库表], [3.2.4节],
  [product\_warehouse], [库存关系表], [3.2.4节],
  [driver\_info], [运输员信息表], [3.2.5节],
  [vehicle], [车辆表], [3.2.5节],
  [order\_info], [订单表], [3.2.6节],
  [delivery], [配送单表], [3.2.6节],
  [hub], [枢纽属性表], [3.2.7节],
  [hub\_link], [运输连接表], [3.2.7节],
  [logistics\_route], [物流路线表], [3.2.7节],
  [logistics\_batch], [配送批次表], [3.2.7节],
  [logistics\_batch\_item], [批次项表], [3.2.7节],
  [dispatch\_pool], [调度池表], [3.2.7节],
  [flow\_plan], [运力规划表], [3.2.7节],
  [flow\_plan\_item], [规划明细表], [3.2.7节],
  header: ([表名], [说明], [正文位置]),
