...
User模型用于保存系统登录身份与基础权限信息，其中permission字段决定用户在网关层和业务层可访问的功能范围。认证服务围绕该模型提供注册、登录、用户查询和权限调整等接口，主要接口设计如表@auth-api 所示。
...
从数据模型看，该服务围绕商户、承运物、仓库和库存关系展开。商户是承运物和仓库的管理主体，其基础属性如表@shop-table 所示。
...
系统采用策略模式将算法选择与业务逻辑解耦，定义`RouteStrategy`接口统一规划入口，支持纯A\*、LLM裁判与LLM路点三种策略的运行时切换。LLM裁判策略适用于候选路线可枚举、需综合评判的场景；LLM路点策略适用于已知拥堵区域但绕行方向需动态建议的场景。两种策略均设计了格式校验与降级机制，确保LLM调用异常时路径规划功能仍可返回确定性路线。
...
在建模方案上，系统以多商品MCMF作为全国干线规划的主要实现。算法层保留了单商品SSP+SPFA实现用于兼容和对比，但正式业务链路按OD对组织跨城运输需求：每个OD对对应一类运输需求，在共享残差容量图上使用Dijkstra-SSP寻找低费用可行路径。求解结果同时包含干线边流量和OD路径，因此能够直接支撑后续inter\_city\_batch生成和订单路径绑定。MCMF 求解的核心步骤如下：
...
运算完成后，系统将规划单总费用、各线路分配流量以及LLM分析报告写入flow\_plan与flow\_plan\_item表中，并依据OD路径结果创建干线批次和订单规划路径，供后续查询、展示与干线执行使用。
...
在接口通信方面，系统将所有HTTP请求统一封装在request.ts中，Axios实例的baseURL设置为/api。请求拦截器从Pinia读取Token并写入`Authorization: Bearer <token>`请求头，使各业务API文件无需重复处理认证信息；响应拦截器统一检查后端返回的业务码，非200结果通过Element Plus消息组件提示用户。若响应状态为401，则说明认证失效或Token过期，前端会清空登录状态并跳转登录页。由此，认证失败、业务错误与网络异常均有统一处理入口，降低了各页面重复编写错误处理逻辑的成本。
...
在镜像构建方面，后端各微服务均采用多阶段Dockerfile。第一阶段使用maven:3.9-eclipse-temurin-17作为构建镜像，复制父POM、common模块、各子模块POM和当前服务源码，通过`mvn package -pl <service> -am -DskipTests`构建目标服务及其依赖模块；第二阶段使用eclipse-temurin:17-jre作为运行镜像，仅复制构建完成的jar包并通过`java -jar app.jar`启动。该方式将Maven构建环境与最终运行环境分离，减小运行镜像体积，也避免在生产运行容器中保留完整源码和构建缓存。
...
前端容器采用Node构建与Nginx运行的两阶段镜像。构建阶段使用node:20.19-alpine执行npm ci和npm run build，生成Vite静态资源；运行阶段使用nginx:1.27-alpine托管dist目录。Nginx配置中通过`try_files $uri $uri/ /index.html`支持Vue Router history模式，使用户直接访问/admin/home等前端路由时仍能回退到单页应用入口；同时将/api/路径反向代理到http://gateway-service:8090，使浏览器访问前端容器时无需知道后端各服务的实际地址。
