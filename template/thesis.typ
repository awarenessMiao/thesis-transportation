...
在订单与履约域，物流订单作为核心聚合实体，关联用户、货主、收货地址、仓库、订单明细、配送批次、物流路线和配送任务等对象，承载从下单、入池、批次生成到配送任务落地的完整链路。图@er-orderdetail 进一步展开订单履约相关实体关系，其中物流订单连接发起用户、承接货主、起点仓库与收货地址，调度池接收待调度订单并生成配送批次，配送批次再生成物流路线并关联配送任务，从而保证订单履约过程中的数据可追溯。
...
在调度与网络规划域，运力规划单、批次规划、物流订单、物流路线、运输员、轨迹点以及中转连接共同支撑跨城干线运输与末端配送协同。图@er-logistics 展示了物流规划相关实体关系：批次规划包含多条物流订单与多条物流路线，物流路线由运输员执行并产生轨迹点；运力规划明细通过中转连接和中转枢纽描述网络中的运输能力分配，为Hub-and-Spoke履约链路和最小费用最大流运力规划提供数据基础。
