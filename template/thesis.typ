...
第三章为系统设计，围绕需求分析、总体架构设计、核心模块设计、前端设计与数据库设计展开。
...
系统核心实体及其关系如下表所示。用户（User）与发货用户（Customer）、商户（Shop）、运输员（Driver）之间为一对一关系，通过user表的permission字段区分角色后，各自的角色专属信息存储在独立表中。发货用户拥有多个地址（Address），形成一对多关系；商户拥有多个承运物（Product）和仓库（Warehouse）；承运物与仓库之间借助库存关系表（ProductWarehouse）关联为多对多。订单（OrderInfo）关联发货用户、商户、地址与仓库，是系统中最核心的聚合实体；配送单（Delivery）关联订单与运输员，区分普通配送与末端配送两种类型；物流路线（LogisticsRoute）关联订单与运输员，存储GeoJSON规划路线数据。在调度层面，批次（LogisticsBatch）包含多个批次项（BatchItem），每个批次项对应一个末端配送订单；枢纽（Hub）之间通过运输连接（HubLink）构成干线网络，支撑MCMF运力规划；调度池（DispatchPool）维护待调度订单，是调度预览与执行的输入数据源；运力规划结果存储在FlowPlan与FlowPlanItem中。
...
本章从需求分析、总体设计、详细设计与数据库设计四个方面对系统进行了系统性说明。基于多角色业务需求明确了功能边界与非功能目标；围绕微服务架构给出了分层架构设计；从认证权限、订单库存、配送路线、调度运力与前端架构五个维度展开详细设计；从全局视角给出了E-R模型与核心表梳理。
...
*测出了什么*：如表和图所示，缓存未命中时平均响应时间为31.2ms，P95达89.4ms（含数据库查询、网络开销与Spring Cache注入延迟）；缓存命中后平均响应时间降至6.0ms，P95降至11.5ms，性能提升约5.2倍。*说明了什么*：这表明基于Redis的Cache-Aside缓存策略对仓库等高频访问数据具有显著的加速效果，减轻了数据库的读取压力。*当前限制*：本次测试在单机环境下进行，缓存命中率受数据量与过期策略影响，在多实例部署场景下还需验证缓存一致性机制的有效性。
...
为验证系统各角色的交互效果，本节展示关键功能页面的运行截图。图@login-register 展示了系统注册与登录页面；图@order-tracking 展示了发货用户下单与物流追踪功能；图@warehouse-inventory 展示了商户库存与运输员任务界面；图@driver-map 展示了导航地图与末端调度预览；图@national-network 展示了全国运力规划结果。

#subimagex(
  grid(columns: 2, column-gutter: 1em,
    image("figures/login-register.png", width: 100%),
    image("figures/product-browse.png", width: 100%),
  ),
  caption: [(a) 用户注册与登录页面 (b) 发起寄件与下单确认页面],
  label-name: "login-register",
)

#subimagex(
  grid(columns: 2, column-gutter: 1em,
    image("figures/order-tracking.png", width: 100%),
    image("figures/shop-products.png", width: 100%),
  ),
  caption: [(a) 订单详情与物流轨迹页面 (b) 商户承运物管理页面],
  label-name: "order-tracking",
)

#subimagex(
  grid(columns: 2, column-gutter: 1em,
    image("figures/warehouse-inventory.png", width: 100%),
    image("figures/driver-tasks.png", width: 100%),
  ),
  caption: [(a) 仓库与库存管理页面 (b) 运输员待处理任务列表],
  label-name: "warehouse-inventory",
)

#subimagex(
  grid(columns: 2, column-gutter: 1em,
    image("figures/driver-map.png", width: 100%),
    image("figures/last-mile-dispatch.png", width: 100%),
  ),
  caption: [(a) 导航与路线地图页面 (b) 末端调度预览与配送批次详情页面],
